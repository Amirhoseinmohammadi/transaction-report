...
| Search | Service implemented / UI pending |
...
The transaction store keeps only the currently requested page of transaction results — it does not accumulate or cache previous pages. The service abstraction decouples the store and UI from the mock data source, making it easier to replace `MockTransactionService` with a real HTTP API without changing the store or component layer.
